...
function loadSavedViews() {
    try {
        ...
        console.log('DEBUG - Loaded saved views:', views);
        return views;
    } catch (e) {
...
function setupViewSelect(retry = 0) {
    ...
    console.log('DEBUG - Current headers:', headers);
    console.log('DEBUG - Current header hash:', headerHash);
    console.log('DEBUG - Available views:', views);
    ...
    let viewsAdded = 0;
    Object.entries(views).forEach(([name, viewObj]) => {
        console.log(`DEBUG - Checking view ${name}:`, viewObj);
        console.log(`DEBUG - View hash: ${viewObj.headerHash}, Current hash: ${headerHash}`);
        if (viewObj.headerHash === headerHash) {
    ...
    console.log(`DEBUG - Views added to dropdown: ${viewsAdded}`);
    ...
    viewSelect.onchange = () => {
        ...
        if (selectedView === '__all__') {
            setVisibleColumns([...getCurrentHeaders()]); // Siempre copia nueva
        } else {
            const viewObj = views[selectedView];
            if (viewObj) {
                // Refuerzo: resetear selección y forzar el orden guardado
                const columnsToShow = [...(viewObj.columnOrder || viewObj.columns)];
                // Aplicar directamente sin setTimeout
                setVisibleColumns(columnsToShow);
                // Restaurar el valor de la búsqueda global
                const globalSearch = getModuleFilterValues()['__globalSearch'];
                if (globalSearch !== undefined) {
                    setModuleFilterValues({ ...getModuleFilterValues(), '__globalSearch': globalSearch });
                    const globalSearchInput = document.getElementById('globalSearchInput');
                    if (globalSearchInput) {
                        globalSearchInput.value = globalSearch;
                        globalSearchInput.dispatchEvent(new Event('input', { bubbles: true }));
                    }
                }
                // Forzar actualización de la tabla y filtros
                applyFilters();
...
// Forzar la actualización del dropdown cuando se carga la página
document.addEventListener('DOMContentLoaded', () => {
    console.log('DEBUG - DOM Content Loaded, setting up view select');
    setupViewSelect();
});
