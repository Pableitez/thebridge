// Booking Hub Manager
class BookingHubManager {
    constructor() {
        this.bookingDashboardModal = null;
        this.bookingSummaryModal = null;
        this.bookingQuickFilters = [];
        this.bookingActiveFilters = [];
        
        // Cards de urgencia para Booking Hub (igual que Ops Hub)
        this.bookingUrgencyCards = [
            { label: 'Critical', key: 'Urgente', color: '#ffcdd2' },
            { label: 'Warning', key: 'Media', color: '#fff9c4' },
            { label: 'Good', key: 'Baja', color: '#c8e6c9' }
        ];
        
        // Estado global de tarjetas activas para Booking Hub
        if (!window.activeBookingUrgencyCards) window.activeBookingUrgencyCards = [];
        
        // Contenedores por defecto para Booking Hub (similar al Ops Hub)
        this.bookingDefaultContainers = {
            'booking-default': 'Booking Zone',
            'booking-container1': 'Pending Bookings',
            'booking-container2': 'Confirmed Bookings',
            'booking-container3': 'Cancelled Bookings',
            'booking-container4': 'Processing Bookings'
        };
    ...
    init() {
        this.bookingDashboardModal = document.getElementById('bookingDashboardModal');
        this.bookingSummaryModal = document.getElementById('bookingSummaryModal');
        
        this.setupEventListeners();
        this.initializeBookingHub();
    }

    setupEventListeners() {
        // Booking Hub toggle button
        const bookingToggleBtn = document.getElementById('bookingDashboardToggleBtn');
        if (bookingToggleBtn) {
            bookingToggleBtn.addEventListener('click', () => this.toggleBookingDashboard());
        }

        // Booking Hub close button
        const bookingCloseBtn = document.getElementById('bookingDashboardCloseBtn');
        if (bookingCloseBtn) {
            bookingCloseBtn.addEventListener('click', () => this.closeBookingDashboard());
        }

        // Booking Hub modal overlay click
        if (this.bookingDashboardModal) {
            this.bookingDashboardModal.addEventListener('click', (e) => {
                if (e.target === this.bookingDashboardModal) {
                    this.closeBookingDashboard();
                }
            });
        }

        // Booking Quality chips
        this.setupBookingQualityChips();

        // Booking Summary generation
        const generateBookingSummaryBtn = document.getElementById('generateBookingSummaryBtn');
        if (generateBookingSummaryBtn) {
            generateBookingSummaryBtn.addEventListener('click', () => this.generateBookingSummary());
        }

        // Booking Summary modal close
        const closeBookingSummaryBtn = document.getElementById('closeBookingSummaryBtn');
        if (closeBookingSummaryBtn) {
            closeBookingSummaryBtn.addEventListener('click', () => this.closeBookingSummary());
        }

        // Booking Summary modal overlay click
        if (this.bookingSummaryModal) {
            this.bookingSummaryModal.addEventListener('click', (e) => {
                if (e.target === this.bookingSummaryModal) {
                    this.closeBookingSummary();
                }
            });
        }

        // Booking Summary actions
        this.setupBookingSummaryActions();
    }

    setupBookingQualityChips() {
        const bookingChips = document.querySelectorAll('#bookingDashboardModal .booking-hub-chip');
        bookingChips.forEach(chip => {
            chip.addEventListener('click', () => {
                // Remove active class from all chips
                bookingChips.forEach(c => c.classList.remove('active'));
    ...
    setupBookingSummaryActions() {
        // Copy to clipboard
        const copyBookingSummaryBtn = document.getElementById('bookingCopySummaryBtn');
        if (copyBookingSummaryBtn) {
            copyBookingSummaryBtn.addEventListener('click', () => this.copyBookingSummaryToClipboard());
        }

        // Export Excel
        const exportBookingExcelBtn = document.getElementById('bookingExportSummaryExcelBtn');
        if (exportBookingExcelBtn) {
            exportBookingExcelBtn.addEventListener('click', () => this.exportBookingSummaryExcel());
        }

        // Report options
        this.setupBookingReportOptions();
    }

    setupBookingReportOptions() {
        // Include table data checkbox
        const includeTableDataCheckbox = document.getElementById('bookingIncludeTableDataCheckbox');
        if (includeTableDataCheckbox) {
            includeTableDataCheckbox.addEventListener('change', () => this.updateBookingSummaryPreview());
        }

        // Include technical info checkbox
        const includeTechnicalInfoCheckbox = document.getElementById('bookingIncludeTechnicalInfoCheckbox');
        if (includeTechnicalInfoCheckbox) {
            includeTechnicalInfoCheckbox.addEventListener('change', () => this.updateBookingSummaryPreview());
        }

        // Remove duplicates checkbox
        const removeDuplicatesCheckbox = document.getElementById('bookingRemoveDuplicatesCheckbox');
        if (removeDuplicatesCheckbox) {
            removeDuplicatesCheckbox.addEventListener('change', () => {
                const duplicateColumnsSection = document.getElementById('bookingDuplicateColumnsSection');
                if (duplicateColumnsSection) {
                    duplicateColumnsSection.style.display = removeDuplicatesCheckbox.checked ? 'block' : 'none';
                }
                this.updateBookingSummaryPreview();
            });
        }

        // Duplicate removal controls
        this.setupBookingDuplicateRemovalControls();
    }

    setupBookingDuplicateRemovalControls() {
        // Select all duplicate columns
        const selectAllBtn = document.getElementById('bookingSelectAllDuplicateColumnsBtn');
        if (selectAllBtn) {
            selectAllBtn.addEventListener('click', () => this.selectAllBookingDuplicateColumns());
        }

        // Deselect all duplicate columns
        const deselectAllBtn = document.getElementById('bookingDeselectAllDuplicateColumnsBtn');
        if (deselectAllBtn) {
            deselectAllBtn.addEventListener('click', () => this.deselectAllBookingDuplicateColumns());
        }


        // Duplicate columns search
        const searchInput = document.getElementById('bookingDuplicateColumnsSearch');
        if (searchInput) {
            searchInput.addEventListener('input', (e) => this.filterBookingDuplicateColumns(e.target.value));
            console.log('✅ Booking Duplicate columns search event listener added');
        } else {
            console.log('⚠️ Booking Duplicate columns search input not found');
        }
    }

    initializeBookingHub() {
        this.renderBookingQuickFilters();
        this.renderBookingUrgencyCards();
        this.updateBookingMetrics();
    }

    renderBookingQuickFilters() {
        const quickFiltersDiv = document.querySelector('#bookingDashboardModal .quickfilters-grid');
        ...
        const quickFilters = this.loadBookingQuickFilters();
        const grouped = {};
        
        // Inicializar los contenedores predeterminados
        Object.entries(this.bookingDefaultContainers).forEach(([key, title]) => {
        ...
        Object.entries(quickFilters).forEach(([name, filterObj]) => {
            ...
            if (!grouped[key]) {
                grouped[key] = { 
                    title: filterObj.containerTitle || key.replace('booking-container', 'Booking Container '),
                    filters: [] 
                };
        ...
        Object.entries(grouped).forEach(([key, group]) => {
            ...
            group.filters.forEach(({ name, filterObj }) => {
                ...
                const updateCardCount = () => {
                    ...
                    const activeFiltersArray = window.activeBookingDashboardQuickFilters || [];
                    ...
                    if (activeFiltersSet.has(name)) {
                        ...
                        // USAR LA MISMA LÓGICA QUE getBookingQuickFilterPreviewCount
                        // Aplicar temporalmente el filtro y usar getFilteredData() que ya tiene toda la lógica correcta
                        const allQuickFilters = this.loadBookingQuickFilters();
                ...
                if (!window.activeBookingDashboardQuickFilters) window.activeBookingDashboardQuickFilters = [];
                const activeFiltersArray = window.activeBookingDashboardQuickFilters;
                ...
                // Evento de click siempre activo, pero verifica compatibilidad dentro
                    ...
                    console.log('🔍 Booking Hub quick filter card clicked:', name);
                    ...
                    if (!sameColumns) {
                        ...
                        if (wasActive) {
                            window.activeBookingDashboardQuickFilters = window.activeBookingDashboardQuickFilters.filter(f => f !== name);
                            // Sincronizar array interno
                            this.bookingActiveFilters = this.bookingActiveFilters.filter(f => f !== name);
                            ...
                            if (!window.activeBookingDashboardQuickFilters) window.activeBookingDashboardQuickFilters = [];
                            window.activeBookingDashboardQuickFilters.push(name);
                            // Sincronizar array interno
                            if (!this.bookingActiveFilters) this.bookingActiveFilters = [];
                            this.bookingActiveFilters.push(name);
                            // Actualizar estado visual INMEDIATAMENTE
                            card.classList.add('active');
                            card.style.background = 'rgba(255, 255, 255, 0.08)';
                            card.style.border = '3px solid rgba(71, 178, 229, 0.85)';
                    ...
                    this.applyBookingCombinedFilters();
                    
                    // Actualizar todos los números en cascada (con debounce)
                    ...
                    const updateAllCounts = () => {
                        ...
                        updateTimeout = setTimeout(() => {
                            const hubModal = document.querySelector('#bookingDashboardModal');
                            ...
                            allCards.forEach((cardEl) => {
                                ...
                                if (cardEl._updateCount && typeof cardEl._updateCount === 'function') {
                                    try {
                                        ...
                                        console.error(`❌ [BOOKING] Error updating count for card ${cardName}:`, error);
                                    }
                                }
                            });
                            updateTimeout = null;
                    ...
                    // Actualizar números después de aplicar filtros (una sola vez)
                    setTimeout(() => {
                        updateAllCounts();
                    }, 150);
                    });
    ...
    renderBookingUrgencyCards() {
        const urgencyCardsContainer = document.querySelector('#bookingDashboardModal .urgency-cards-container');
        if (!urgencyCardsContainer) return;

        // Limpiar contenedor
        urgencyCardsContainer.innerHTML = '';

        // Cargar quick filters del Booking Hub
        const quickFilters = this.loadBookingQuickFilters();
        ...
        const getPreviewCount = (cardKey) => {
            ...
            let data = [];
            if (window.activeBookingUrgencyCards && window.activeBookingUrgencyCards.length > 0) {
            ...
            const isActive = window.activeBookingUrgencyCards.includes(cardKey);
            const previewCards = isActive
                ? window.activeBookingUrgencyCards
                : [...window.activeBookingUrgencyCards, cardKey];
        ...
        this.bookingUrgencyCards.forEach(card => {
            const btn = document.createElement('button');
            btn.textContent = card.label;
            btn.className = 'booking-hub-chip';
            btn.setAttribute('data-urgency', card.key);
            
            const isActive = window.activeBookingUrgencyCards.includes(card.key);
            ...
            btn.addEventListener('click', () => {
                ...
                if (isActive) {
                    window.activeBookingUrgencyCards = window.activeBookingUrgencyCards.filter(k => k !== card.key);
                    // Show notification for deactivated urgency card
                    if (typeof window.showUnifiedNotification === 'function') {
                        window.showUnifiedNotification(`Urgency card "${card.label}" deactivated!`, 'info');
                    }
                } else {
                    window.activeBookingUrgencyCards.push(card.key);
                ...
                this.applyBookingUrgencyFilters();
    ...
    toggleBookingDashboard() {
        if (this.bookingDashboardModal) {
            const isOpening = this.bookingDashboardModal.classList.contains('hidden');
            this.bookingDashboardModal.classList.toggle('hidden');
            if (!this.bookingDashboardModal.classList.contains('hidden') && isOpening) {
    ...
    closeBookingDashboard() {
        if (this.bookingDashboardModal) {
            this.bookingDashboardModal.classList.add('hidden');
        }
    }

    closeBookingSummary() {
        if (this.bookingSummaryModal) {
            this.bookingSummaryModal.classList.add('hidden');
        }
    }
    ...
    updateBookingMetrics() {
        // Calculate and update Booking metrics based on current data
        ...
        this.updateMetric('booking-duplicates', 'Total Duplicates', duplicates.total);
        this.updateMetric('booking-duplicates', 'Duplicate Rate', `${((duplicates.total / data.length) * 100).toFixed(1)}%`);
        this.updateMetric('booking-duplicates', 'Unique Records', data.length - duplicates.total);

        // Calculate null values
        const nulls = this.calculateNullValues(data);
        this.updateMetric('booking-nulls', 'Null Records', nulls.total);
        this.updateMetric('booking-nulls', 'Null Rate', `${((nulls.total / (data.length * Object.keys(data[0] || {}).length)) * 100).toFixed(1)}%`);
        this.updateMetric('booking-nulls', 'Complete Records', data.length - nulls.recordsWithNulls);

        // Calculate format issues
        const formatIssues = this.calculateFormatIssues(data);
        this.updateMetric('booking-format', 'Format Errors', formatIssues.total);
        this.updateMetric('booking-format', 'Error Rate', `${((formatIssues.total / (data.length * Object.keys(data[0] || {}).length)) * 100).toFixed(1)}%`);
        this.updateMetric('booking-format', 'Valid Records', data.length - formatIssues.recordsWithErrors);

        // Calculate completeness
        const completeness = this.calculateCompleteness(data);
        this.updateMetric('booking-completeness', 'Completeness Score', `${completeness.score.toFixed(1)}%`);
        this.updateMetric('booking-completeness', 'Missing Fields', completeness.missingFields);
        this.updateMetric('booking-completeness', 'Complete Fields', completeness.completeFields);

        // Calculate consistency
        const consistency = this.calculateConsistency(data);
        this.updateMetric('booking-consistency', 'Inconsistencies', consistency.inconsistencies);
        this.updateMetric('booking-consistency', 'Consistency Score', `${consistency.score.toFixed(1)}%`);
        this.updateMetric('booking-consistency', 'Consistent Records', data.length - consistency.recordsWithIssues);

        // Calculate accuracy
        const accuracy = this.calculateAccuracy(data);
        this.updateMetric('booking-accuracy', 'Accuracy Score', `${accuracy.score.toFixed(1)}%`);
        this.updateMetric('booking-accuracy', 'Validation Errors', accuracy.errors);
        this.updateMetric('booking-accuracy', 'Valid Records', data.length - accuracy.recordsWithErrors);
    }
    ...
    loadBookingQuickFilters() {
        try {
            const saved = localStorage.getItem('quickFilters');
            const allFilters = saved ? JSON.parse(saved) : {};
            
            // Also load duplicate detection filters for Booking Hub
            const duplicateFilters = JSON.parse(localStorage.getItem('duplicateQuickFilters') || '{}');
            const bookingDuplicateFilters = Object.entries(duplicateFilters)
                .filter(([name, filter]) => filter.hubType === 'booking')
            ...
            // Filter only Booking Hub filters and combine with duplicate filters
            ...
            return { ...regularFilters, ...bookingDuplicateFilters };
        } catch (error) {
            console.error('Error loading Booking quick filters:', error);
            return {};
        }
    }

    getBookingQuickFilterPreviewCount(name) {
    ...
    applyBookingQuickFilters() {
        this.applyBookingCombinedFilters();
    }

    applyBookingCombinedFilters() {
        const data = window.getOriginalData ? window.getOriginalData() : [];
        if (!data || !data.length) {
        ...
        const quickFilters = this.loadBookingQuickFilters();
        const activeNames = this.bookingActiveFilters || [];
        const activeUrgencyCards = window.activeBookingUrgencyCards || [];
        let combinedFilterValues = {};
        let combinedActiveFilters = {};
    ...
    generateBookingSummary() {
        if (this.bookingSummaryModal) {
            this.bookingSummaryModal.classList.remove('hidden');
            this.updateBookingSummaryPreview();
        }
    }

    updateBookingSummaryPreview() {
        const previewContainer = document.getElementById('bookingSummaryPreview');
        if (!previewContainer) return;

        const summary = this.generateBookingSummaryContent();
        previewContainer.innerHTML = summary;
    }

    generateBookingSummaryContent() {
        const data = window.getCurrentData ? window.getCurrentData() : [];
        const includeTableData = document.getElementById('bookingIncludeTableDataCheckbox')?.checked || false;
        const includeTechnicalInfo = document.getElementById('bookingIncludeTechnicalInfoCheckbox')?.checked || false;
        const removeDuplicates = document.getElementById('bookingRemoveDuplicatesCheckbox')?.checked || false;

        let summary = `
            <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
                <h1 style="color: #000000; border-bottom: 2px solid #47B2E5; padding-bottom: 10px;">Booking Summary Report</h1>
        ...
        if (includeTechnicalInfo) {
            summary += `
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    ...
                    <p><strong>Report Type:</strong> Booking Analysis</p>
                </div>
            `;
        }

        // Add Booking metrics summary
        summary += this.generateBookingMetricsSummary(data);

        if (includeTableData && data.length > 0) {
            summary += this.generateBookingTableData(data, removeDuplicates);
        }
    ...
    generateBookingMetricsSummary(data) {
        const duplicates = this.calculateDuplicates(data);
        const nulls = this.calculateNullValues(data);
        const formatIssues = this.calculateFormatIssues(data);
        const completeness = this.calculateCompleteness(data);
        ...
        return `
            <div style="margin-bottom: 30px;">
                <h2 style="color: #000000; border-bottom: 1px solid #ddd; padding-bottom: 5px;">Booking Metrics</h2>
    ...
    generateBookingTableData(data, removeDuplicates) {
    ...
    copyBookingSummaryToClipboard() {
        ...
        this.showNotification('Booking Summary copied to clipboard!', 'success');
    }

    exportBookingSummaryPdf() {
        this.showNotification('PDF export functionality will be implemented soon.', 'info');
    }

    exportBookingSummaryExcel() {
        // Use the BookingHubSummary export function
        if (window.bookingHubSummary && typeof window.bookingHubSummary.exportToExcel === 'function') {
            window.bookingHubSummary.exportToExcel();
        } else {
            this.showNotification('Excel export functionality is not available. Please try again.', 'error');
        }
    }

    selectAllBookingDuplicateColumns() {
        const checkboxes = document.querySelectorAll('#bookingDuplicateColumnsList .duplicate-column-checkbox');
        checkboxes.forEach(checkbox => checkbox.checked = true);
        this.updateBookingSelectedFieldsCount();
    }

    deselectAllBookingDuplicateColumns() {
        const checkboxes = document.querySelectorAll('#bookingDuplicateColumnsList .duplicate-column-checkbox');
        checkboxes.forEach(checkbox => checkbox.checked = false);
        this.updateBookingSelectedFieldsCount();
    }

    selectCommonBookingFields() {
        // Implement common fields selection logic
        this.showNotification('Common fields selection will be implemented soon.', 'info');
    }

    filterBookingDuplicateColumns(searchTerm) {
        console.log('🔍 Filtering Booking duplicate columns with term:', searchTerm);
        const checkboxes = document.querySelectorAll('#bookingDuplicateColumnsList .duplicate-column-checkbox');
        
        if (checkboxes.length === 0) {
            console.log('⚠️ No checkboxes found in bookingDuplicateColumnsList');
            return;
        }
        
        let visibleCount = 0;
        checkboxes.forEach(checkbox => {
            // En la estructura del Booking Hub, el checkbox está dentro de un label
    ...
    updateBookingSelectedFieldsCount() {
        const checkboxes = document.querySelectorAll('#bookingDuplicateColumnsList .duplicate-column-checkbox:checked');
        const countSpan = document.getElementById('bookingSelectedFieldsCount');
        if (countSpan) {
            countSpan.textContent = checkboxes.length;
        }
    }

    applyBookingUrgencyFilters() {
        const data = window.getOriginalData ? window.getOriginalData() : [];
        if (!data || !data.length) {
        ...
        // Actualizar métricas del Booking Hub
        this.updateBookingMetrics();
        ...
        if (window.analyticsDashboard && window.analyticsDashboard.isOpen()) {
            console.log('🔄 Triggering analytics update from Booking Hub filters...');
            setTimeout(() => {
        ...
        window.dispatchEvent(new CustomEvent('filtersChanged', { 
            detail: { 
                activeQuickFilters: [],
                activeUrgencyCards: window.activeBookingUrgencyCards || [],
                hubType: 'booking'
            } 
        }));
    }
    ...
    showSavePreferencesModal() {
        ...
        modal.id = 'bookingSavePreferencesModal';
        ...
        const currentPrefs = {
            includeTableData: document.getElementById('bookingIncludeTableDataCheckbox')?.checked || false,
            includeTechnicalInfo: document.getElementById('bookingIncludeTechnicalInfoCheckbox')?.checked || false,
            removeDuplicates: document.getElementById('bookingRemoveDuplicatesCheckbox')?.checked || false,
            duplicateColumns: Array.from(document.querySelectorAll('#bookingDuplicateColumnsList .duplicate-column-checkbox:checked')).map(cb => cb.value)
        };
        
        // Get current state for complete preview
        const selectedCards = window.bookingHubSummary ? Array.from(window.bookingHubSummary.selectedCards) : [];
        const selectedCardsNames = selectedCards.map(cardId => {
            return window.bookingHubSummary?.quickCardsData[cardId]?.name || cardId;
        });
        const activeFilters = window.activeBookingQuickFilters || [];
        const activeUrgency = window.activeBookingUrgencyCards || [];
        ...
        content.innerHTML = `
            <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; padding: 1.5rem 2rem; border-bottom: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05) !important;">
                <div class="header-left" style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <h3 style="color: #000000; margin: 0; font-size: 1.3rem;">Save Complete Booking Preferences</h3>
                </div>
            ...
            <div class="modal-content" style="padding: 2rem; background: transparent !important;">
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <input type="text" id="preferenceNameInput" 
                           ...
                           value="Booking Preferences ${new Date().toLocaleDateString()}">
                </div>
            ...
            <div class="modal-footer" style="padding: 1.5rem 2rem; border-top: 1px solid rgba(0,0,0,0.1); background: rgba(0,0,0,0.05) !important; display: flex; justify-content: flex-end; gap: 1rem;">
                ...
                <button class="modal-btn primary" onclick="window.bookingHubManager.confirmSavePreferences()" style="background: #47B2E5 !important; border: 1px solid #47B2E5 !important; color: #FFFFFF !important; padding: 0.75rem 1.5rem; border-radius: 6px; cursor: pointer; transition: all 0.3s ease; opacity: 1 !important;">Save Complete Preferences</button>
            </div>
        `;
        ...
        document.addEventListener('keydown', function saveOnEnter(e) {
            if (e.key === 'Enter') {
                window.bookingHubManager.confirmSavePreferences();
    ...
    confirmSavePreferences() {
        try {
            ...
            const preferenceName = nameInput?.value?.trim() || `Booking Preferences ${new Date().toLocaleDateString()}`;
            ...
            const cardCheckboxes = document.querySelectorAll('#bookingQuickCardsSelection .card-checkbox-input:checked');
            ...
            if (selectedCards.length === 0 && window.bookingHubSummary && window.bookingHubSummary.selectedCards) {
                window.bookingHubSummary.selectedCards.forEach(cardId => {
                    selectedCards.push(cardId);
                    
                    // Get configuration from internal state
                    if (window.bookingHubSummary.quickCardsData[cardId]) {
                        const cardData = window.bookingHubSummary.quickCardsData[cardId];
                        const cardConfig = window.bookingHubSummary.getCardConfig(cardId);
            ...
            if (selectedCards.length === 0 && window.bookingHubSummary && window.bookingHubSummary.quickCardsData) {
                Object.keys(window.bookingHubSummary.quickCardsData).forEach(cardId => {
                    selectedCards.push(cardId);
            ...
            const preferences = {
                ...
                // Quick filters state
                activeQuickFilters: window.activeBookingQuickFilters || [],
                activeUrgencyCards: window.activeBookingUrgencyCards || [],
            ...
            const existingPrefs = JSON.parse(localStorage.getItem('bookingReportPreferences') || '[]');
            ...
            localStorage.setItem('bookingReportPreferences', JSON.stringify(existingPrefs));
            
            // Close modal
            const modal = document.querySelector('#bookingSavePreferencesModal');
            if (modal) modal.remove();
            
            this.showNotification(`Preferences saved as "${preferences.name}"`, 'success');
            console.log('✅ Booking Preferences saved:', preferences);
        } catch (error) {
    ...
    showPreferencesSelectionModal(preferences) {
        ...
        const modal = document.createElement('div');
        modal.className = 'modal-overlay';
        modal.id = 'bookingPreferencesModal';
        modal.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.1); z-index: 10000; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(4px);';
                ...
                <div class="preferences-grid" style="display: grid; gap: 1rem;">
                    ${preferences.map((pref, index) => `
                        <div class="preference-item" 
                             ...
                             onclick="window.bookingHubManager.applyBookingPreferences(${index})">
                            ...
                            <div style="position: absolute; bottom: 1rem; right: 1rem; opacity: 0; transition: opacity 0.3s ease;" class="action-buttons">
                                            ...
                                            onclick="event.stopPropagation(); window.bookingHubManager.deleteBookingPreference(${index})"
    ...
    deleteBookingPreference(prefIndex) {
        try {
            const existingPrefs = JSON.parse(localStorage.getItem('bookingReportPreferences') || '[]');
            ...
            if (prefIndex >= 0 && prefIndex < existingPrefs.length) {
                ...
                const modal = document.getElementById('bookingPreferencesModal');
                if (modal) modal.remove();
    ...
    applyBookingPreferences(prefIndex) {
        try {
            ...
            console.log('🔄 Applying complete Booking preferences:', preferences);
            ...
            if (window.bookingHubSummary && preferences.selectedCards) {
                // Clear current selection
                window.bookingHubSummary.selectedCards.clear();
                
                // Add selected cards
                preferences.selectedCards.forEach(cardId => {
                    window.bookingHubSummary.selectedCards.add(cardId);
                });
                ...
                if (preferences.cardConfigurations) {
                    Object.entries(preferences.cardConfigurations).forEach(([cardId, config]) => {
                        if (window.bookingHubSummary.quickCardsData[cardId]) {
                            ...
                            if (config.viewType) {
                                window.bookingHubSummary.updateCardConfig(cardId, 'viewType', config.viewType);
                            }
                            if (config.savedView) {
                                window.bookingHubSummary.updateCardConfig(cardId, 'savedView', config.savedView);
                            }
                            // Also update the internal cardConfigs directly to ensure persistence
                            if (!window.bookingHubSummary.cardConfigs[cardId]) {
                                window.bookingHubSummary.cardConfigs[cardId] = {};
                            }
                            if (config.viewType) {
                                window.bookingHubSummary.cardConfigs[cardId].viewType = config.viewType;
                            }
                            if (config.savedView) {
                                window.bookingHubSummary.cardConfigs[cardId].savedView = config.savedView;
                            }
                        }
                    });
                }
                
                // Ensure Booking Summary modal is open to show the cards
                if (!document.getElementById('bookingSummaryModal')) {
                    this.toggleBookingDashboard();
                }
                
                // Re-render cards selection to update UI
                window.bookingHubSummary.renderCardsSelection();
                
                // Force update the UI to reflect the selected cards with multiple attempts
                setTimeout(() => {
                    if (window.bookingHubSummary && window.bookingHubSummary.renderCardsSelection) {
                        window.bookingHubSummary.renderCardsSelection();
            ...
            if (preferences.activeQuickFilters) {
                window.activeBookingQuickFilters = preferences.activeQuickFilters;
                // Re-apply quick filters
                this.applyBookingQuickFilters();
            }
            
            // Apply urgency cards state
            if (preferences.activeUrgencyCards) {
                window.activeBookingUrgencyCards = preferences.activeUrgencyCards;
            ...
            console.log('✅ Complete Booking Preferences applied:', preferences);
        } catch (error) {
            console.error('Error applying Booking preferences:', error);
            this.showNotification('Error applying preferences. Please try again.', 'error');
        }
    }
}

// Initialize Booking Hub Manager when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    window.bookingHubManager = new BookingHubManager();
}); 