...
export class BookingHubSummary {
    constructor() {
    ...
    window.bookingFilterDuplicateColumns = () => {
      if (window.bookingHubSummaryInstance) {
        window.bookingHubSummaryInstance.filterDuplicateColumns();
      }
    };
  }
  ...
  bindEvents() {
    ...
    const generateBtn = document.getElementById('generateBookingSummaryBtn');
    if (generateBtn) {
      generateBtn.addEventListener('click', () => this.openSummaryModal());
    }

    // Modal close button
    const closeBtn = document.getElementById('closeBookingSummaryBtn');
    if (closeBtn) {
      closeBtn.addEventListener('click', () => this.closeSummaryModal());
    }

    // Action buttons
    const copyBtn = document.getElementById('bookingCopySummaryBtn');
    const pdfBtn = document.getElementById('bookingExportSummaryPdfBtn');
    const excelBtn = document.getElementById('bookingExportSummaryExcelBtn');
  ...
  openSummaryModal() {
    console.log('Booking Opening summary modal...');
    
    // Expose instance globally for HTML event handlers
    window.bookingHubSummaryInstance = this;
    
    // Restore current state before opening
    this.restoreCurrentState();
    
    // Force refresh of quick filters data before collecting
    if (typeof window.renderBookingDashboardQuickFilters === 'function') {
      console.log('Booking Refreshing quick filters...');
      window.renderBookingDashboardQuickFilters();
    }
    
    // Wait a bit for the DOM to update, then collect data
    setTimeout(() => {
      console.log('Booking Setting up modal content...');
      ...
      console.log('Booking Setting up event listeners...');
      this.setupModalEventListeners();
      
      // FORCE POPULATE COLUMNS IMMEDIATELY
      console.log('Booking FORCING populateDuplicateColumns...');
  ...
  setupModalEventListeners() {
    console.log('Setting up Booking modal event listeners...');
    ...
    // Duplicate removal
      ...
      console.log('Booking Found removeDuplicatesCheckbox, setting up event listener');
            removeDuplicatesCheckbox.addEventListener('change', () => {
        console.log('Booking Checkbox change event triggered');
        this.handleDuplicateRemovalChange();
      });
    } else {
      console.log('Booking ERROR: removeDuplicatesCheckbox not found');
    }

    // Duplicate columns controls
    const selectAllColumnsBtn = document.getElementById('bookingSelectAllDuplicateColumnsBtn');
    const deselectAllColumnsBtn = document.getElementById('bookingDeselectAllDuplicateColumnsBtn');
    const selectCommonFieldsBtn = document.getElementById('bookingSelectCommonFieldsBtn');
  ...
  saveCurrentState() {
    try {
      ...
      localStorage.setItem('bookingSummaryCurrentState', JSON.stringify(currentState));
      console.log('Booking Current state saved:', currentState);
    } catch (error) {
      console.error('Booking Error saving current state:', error);
    }
  }
  
  restoreCurrentState() {
    try {
      const savedState = localStorage.getItem('bookingSummaryCurrentState');
      if (savedState) {
        ...
        console.log('Booking Current state restored:', state);
        return true;
      }
    } catch (error) {
      console.error('Booking Error restoring current state:', error);
    }
    return false;
  }
  ...
  collectQuickCardsData() {
    console.log('Collecting Booking quick cards data...');
    this.quickCardsData = {};
    this.currentPriority = '';

    // Get current priority filter
    const activeChip = document.querySelector('#bookingDashboardModal .booking-hub-chip.active');
    if (activeChip) {
      this.currentPriority = activeChip.getAttribute('data-urgency');
      console.log('Current Booking priority filter:', this.currentPriority);
    } else {
      console.log('No active Booking priority filter found');
    }

    // Collect data from quick cards - look for kpi-card elements in quickfilter containers
    let quickCards = document.querySelectorAll('#bookingDashboardModal .quickfilter-cards-container .kpi-card');
    
    // If no cards found, try alternative selectors
    if (quickCards.length === 0) {
      quickCards = document.querySelectorAll('#bookingDashboardModal .quickfilters-grid .kpi-card');
    }
    
    // If still no cards, try any kpi-card in the Booking dashboard
    if (quickCards.length === 0) {
      quickCards = document.querySelectorAll('#bookingDashboardModal .kpi-card');
    }
    ...
    quickCards.forEach(card => {
      ...
      if (!cardName) {
        cardName = 'Booking Quick Filter Card';
      }
    ...
    if (Object.keys(this.quickCardsData).length === 0) {
      this.quickCardsData['no_cards'] = {
        id: 'no_cards',
        name: 'No Booking Quick Cards Available',
        count: 0,
        active: false,
        element: null
      };
    ...
    console.log('Collected Booking quick cards data:', this.quickCardsData);
    console.log('Total Booking cards found:', Object.keys(this.quickCardsData).length);
  }

  generateCardId(card) {
    const title = card.querySelector('.kpi-title')?.textContent || '';
    return `booking_card_${title.toLowerCase().replace(/[^a-z0-9]/g, '_')}_${Date.now()}`;
  }

  getBookingQuickFilterPreviewCount(filterName) {
    // Get preview count for Booking quick filter
    if (typeof window.getBookingQuickFilterPreviewCount === 'function') {
      return window.getBookingQuickFilterPreviewCount(filterName);
    }
    return '0';
  }
  ...
  updateSummaryInfo() {
    ...
    console.log(`Booking Summary Info: ${activeCards}/${totalCards} cards active`);
  }

  renderCardsSelection() {
    const cardsContainer = document.getElementById('bookingQuickCardsSelection');
  ...
  createCardSelectionItem(cardData) {
    ...
    checkbox.addEventListener('change', (e) => {
      e.stopPropagation();
      console.log('Booking Checkbox changed for card:', cardData.id, 'checked:', checkbox.checked);
  ...
  toggleCardSelection(cardId, element) {
    console.log('Booking toggleCardSelection called for:', cardId);
    ...
    if (this.selectedCards.has(cardId)) {
      ...
      console.log('Booking Card deselected:', cardId);
    } else {
      this.selectedCards.add(cardId);
      ...
      console.log('Booking Card selected:', cardId);
    }
  ...
  _renderSummaryPreviewInternal() {
    const preview = document.getElementById('bookingSummaryPreview');
    if (!preview || this.isGeneratingPreview) {
      console.log('Booking Preview update skipped:', { preview: !!preview, isGenerating: this.isGeneratingPreview });
      return;
    }

    this.isGeneratingPreview = true;
    console.log('Booking Starting preview generation...');
    
    try {
      const selectedCardsData = Object.values(this.quickCardsData)
        .filter(card => this.selectedCards.has(card.id));

      console.log('Booking Rendering preview with', selectedCardsData.length, 'selected cards');
      
      // Log card configurations for debugging
      selectedCardsData.forEach(card => {
        const config = this.getCardConfig(card.id);
        console.log(`Booking Card "${card.name}" config:`, config);
      });
      ...
      requestAnimationFrame(() => {
        ...
        this.isGeneratingPreview = false;
        console.log('Booking Preview generation completed');
      });
    } catch (error) {
      console.error('Booking Error generating preview:', error);
  ...
  generateSeparateTablesForCards(selectedCards, config) {
    ...
    selectedCards.forEach((card, index) => {
      console.log(`Booking Processing card ${index + 1}/${selectedCards.length}: "${card.name}"`);
      
      const cardConfig = this.getCardConfig(card.id);
      console.log(`Booking Card config:`, cardConfig);
      
      const cardData = this.getDataForQuickCardWithConfig(card, cardConfig);
      console.log(`Booking Card data received:`, cardData?.length || 0, 'records');
      console.log(`Booking Card data type:`, typeof cardData);
      console.log(`Booking Card data is array:`, Array.isArray(cardData));
      if (cardData && cardData.length > 0) {
        console.log(`Booking First record sample:`, cardData[0]);
      }
      ...
      if (cardData && cardData.length > 0) {
        ...
        console.log(`Booking Processing card "${card.name}": ${originalCount} original records`);
        ...
        console.log(`Booking Card "${card.name}" after duplicate removal: ${processedData.length} records (removed ${removedCount})`);
        ...
        if (cardConfig.viewType === 'summary') {
          ...
          if (config.includeTableData) {
            console.log(`Booking Generating table for card "${card.name}" with ${processedData.length} records`);
            const tableConfig = {
              ...cardViewConfig,
        ...
        console.log(`Booking No data available for card "${card.name}"`);
        tablesHtml += `
          <div style="margin-bottom:2rem;">
            <h3 style="font-size:1.1rem; color:#15364A; margin-bottom:0.5rem; border-bottom:1px solid #e3f2fd; padding-bottom:0.3rem;">
              ${card.name}
    ...
    console.log(`Booking generateSeparateTablesForCards completed: ${totalRecords} total records from ${selectedCards.length} cards`);
  ...
  generateTableDataSection(tableData, config, customTitle = 'Table Data') {
    console.log(`Booking generateTableDataSection called for "${customTitle}":`, {
    ...
    if (config.savedView && config.savedView !== 'current') {
      ...
      if (selectedView && selectedView.columns) {
        console.log(`Booking Applying saved view columns "${config.savedView}" to table:`, selectedView.columns);
        ...
        console.log(`Booking Filtered headers for saved view:`, headers);
      }
    }
  ...
  getDataForCurrentConfiguration(config) {
    ...
    // If using a saved view, apply that view to the data
      ...
      if (selectedView && selectedView.columns) {
        console.log(`Booking Applying saved view "${config.tableView}" to current configuration:`, selectedView);
        ...
        console.log(`Booking Data after applying saved view "${config.tableView}":`, data.length, 'records');
        result = data;
      } else {
        console.warn(`Booking Saved view "${config.tableView}" not found or invalid.`);
        result = this.getOriginalData(); // Use original data, not filtered
      }
    }
    // Default: use original data for Booking Hub (PRUEBA)
    else {
      result = this.getOriginalData(); // Usar los datos originales para Booking Hub
    }
    
    // Apply duplicate removal if enabled
    if (config.removeDuplicates && config.duplicateColumns.length > 0) {
      console.log('Booking Applying duplicate removal to current configuration data');
  ...
  getDataForQuickCardWithConfig(cardData, cardConfig) {
    // PRUEBA: Usar datos originales en lugar de filtrados para Booking Hub
    console.log('Booking getDataForQuickCardWithConfig called for:', cardData.name);
    console.log('Booking window.getFilteredData available:', !!window.getFilteredData);
    console.log('Booking window.getOriginalData available:', !!window.getOriginalData);
    
    // PRUEBA: Usar datos originales para Booking Hub
    let filteredData = this.getOriginalData();
    console.log(`Booking Initial filteredData length: ${filteredData.length}`);
    
    // Apply saved view if specified and not "current"
    if (cardConfig && cardConfig.savedView && cardConfig.savedView !== 'current') {
      console.log(`Booking Applying saved view "${cardConfig.savedView}" to card "${cardData.name}"`);
      
      const savedViews = this.loadSavedViews();
      const selectedView = savedViews[cardConfig.savedView];
      
      if (selectedView && selectedView.columns) {
        console.log(`Booking Found saved view "${cardConfig.savedView}":`, selectedView);
        ...
        console.log(`Booking Applied saved view "${cardConfig.savedView}": ${filteredData.length} records have values in all specified columns`);
            } else {
        console.warn(`Booking Saved view "${cardConfig.savedView}" not found or invalid`);
      }
    }
    
    console.log(`Booking Looking for quick filter for card: "${cardData.name}" (ID: ${cardData.id})`);
    
    // Use Booking-specific quick filters instead of general quick filters
    console.log('Booking window.bookingHubManager available:', !!window.bookingHubManager);
    const bookingQuickFilters = window.bookingHubManager ? window.bookingHubManager.loadBookingQuickFilters() : {};
    console.log('Booking Available Booking quick filters:', Object.keys(bookingQuickFilters));
    console.log('Booking Booking quick filters content:', bookingQuickFilters);
    
    // Find the quick filter that corresponds to this card
    const filterEntry = Object.entries(bookingQuickFilters).find(([name, obj]) => {
      console.log(`Booking Checking filter "${name}":`, obj);
      ...
      if (obj.linkedUrgencyCard && obj.linkedUrgencyCard === cardData.id) {
        console.log(`Booking ✓ Matched by linkedUrgencyCard: ${obj.linkedUrgencyCard} === ${cardData.id}`);
        return true;
      }
      
      // Priority 2: Match by exact name
      if (obj.name && obj.name === cardData.name) {
        console.log(`Booking ✓ Matched by exact name: "${obj.name}" === "${cardData.name}"`);
        return true;
      }
      
      // Priority 3: Match by filter name containing card name
      if (name.toLowerCase().includes(cardData.name.toLowerCase())) {
        console.log(`Booking ✓ Matched by filter name contains card name: "${name}" contains "${cardData.name}"`);
        return true;
      }
      
      // Priority 4: Match by obj.name containing card name
      if (obj.name && obj.name.toLowerCase().includes(cardData.name.toLowerCase())) {
        console.log(`Booking ✓ Matched by obj.name contains card name: "${obj.name}" contains "${cardData.name}"`);
        return true;
      }
      ...
      if (cardUrgency) {
        ...
        if (matchesUrgency) {
          console.log(`Booking ✓ Matched by urgency level: "${cardData.name}" matches "${name}" or "${obj.name}"`);
          return true;
        }
      }
    ...
    if (filterEntry) {
      ...
      console.log(`Booking ✓ Found matching quick filter: "${filterName}" for card "${cardData.name}"`);
      console.log('Booking Filter object:', filterObj);
      ...
      console.log(`Booking Card shows ${cardData.count} records, filter returned ${bestResult.length} records`);
      if (cardData.count !== bestResult.length) {
        console.warn(`Booking ⚠️ Count mismatch: Card shows ${cardData.count} but filter returned ${bestResult.length}`);
      }
      
      console.log(`Booking Returning bestResult with ${bestResult.length} records for card "${cardData.name}"`);
      return bestResult;
    }

    // Fallback to filtered data (not original) - EXACTAMENTE como Ops Hub
    console.log(`Booking ❌ No quick filter found for card "${cardData.name}", using filtered data`);
    console.log(`Booking Returning filteredData with ${filteredData.length} records for card "${cardData.name}"`);
    return filteredData;
  }

  applyQuickFilterWithSystemLogic(filterName, filterObj, baseData) {
    // Soporta quick filters de duplicados para el preview del Booking Summary
    const data = baseData;
    ...
    if (!window.getModuleActiveFilters || !window.setModuleActiveFilters || 
      ...
      console.warn('Booking System filter functions not available, using fallback method');
    ...
    console.log(`Booking Quick filter "${filterName}" applied with system logic: ${data.length} -> ${count} records`);
  ...
  applyQuickFilterToData(filterName, filterObj, baseData) {
    ...
    if (!filterObj || !filterObj.filterValues) {
      console.log(`Booking No filterValues in filterObj for "${filterName}"`, filterObj);
      return originalData;
    }
    ...
    Object.entries(filterObj.filterValues).forEach(([column, filterValue]) => {
      if (filterValue && filterValue !== '') {
        ...
        console.log(`Booking Filter "${column} = ${filterValue}": ${beforeCount} -> ${afterCount} records`);
      }
    });
    
    console.log(`Booking Quick filter "${filterName}" applied (fallback): ${originalData.length} -> ${filteredData.length} records`);
    return filteredData;
  }

  getOriginalData() {
    console.log('Booking Getting original data...');
    ...
    if (window.getOriginalData && typeof window.getOriginalData === 'function') {
      ...
      console.log(`Booking Got original data from window.getOriginalData(): ${data.length} records`);
      return data;
    }
    ...
    if (table) {
      ...
      if (rows.length > 0) {
        ...
        console.log(`Booking Got original data from table: ${data.length} records`);
        return data;
      }
    }
    ...
    if (window.tableData && Array.isArray(window.tableData)) {
      console.log(`Booking Got original data from window.tableData: ${window.tableData.length} records`);
      return window.tableData;
    }
    
    // Try to get from CSV data if available
    if (window.csvData && Array.isArray(window.csvData)) {
      console.log(`Booking Got original data from window.csvData: ${window.csvData.length} records`);
      return window.csvData;
    }
    
    console.warn('Booking Could not get original data, using empty array');
    return [];
  }
  ...
  updateCardConfig(cardId, key, value) {
    ...
    console.log(`Booking Updated card config for ${cardId}:`, this.cardConfigs[cardId]);
  }
  ...
  loadSavedViews() {
    try {
      ...
      console.error('Booking Error loading saved views:', error);
      return {};
    }
  }
    ...
    try {
      ...
      if (navigator.clipboard && navigator.clipboard.write) {
        try {
          ...
          this.showNotification('Booking summary copied to clipboard with formatting!', 'success');
          return;
        } catch (htmlError) {
  ...
  exportToExcel() {
    try {
      ...
      const configData = [
        { 'Setting': 'Report Type', 'Value': 'Booking Summary Report' },
      ...
      // Save the file
      const fileName = `Booking_Summary_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  generateCleanTextContent() {
    const selectedCards = Array.from(this.selectedCards);
    if (selectedCards.length === 0) {
      return 'No cards selected for Booking summary report.';
    }

    const config = this.getReportConfig();
    const dateStr = new Date().toLocaleString();
    
    let content = `Booking Summary Report\n`;
  ...
  removeDuplicatesFromData(data, config = null) {
    ...
    console.log('Booking removeDuplicatesFromData called with:', {
    ...
    if (!reportConfig.removeDuplicates || reportConfig.duplicateColumns.length === 0) {
      console.log('Booking Duplicate removal not enabled or no columns selected, returning original data');
      return data;
    }
    ...
    console.log(`Booking Duplicate removal applied: ${data.length} -> ${filteredData.length} records (removed ${data.length - filteredData.length})`);
    return filteredData;
  }
  ...
  populateDuplicateColumns() {
    const duplicateColumnsList = document.getElementById('bookingDuplicateColumnsList');
    
    if (!duplicateColumnsList) {
      console.log('Booking ERROR: duplicateColumnsList element not found!');
      return;
    }

    console.log('Booking populateDuplicateColumns called - starting...');

    // Get data from selected cards or use original data as fallback
    const selectedCards = Object.values(this.quickCardsData).filter(card => this.selectedCards.has(card.id));
    console.log('Booking Selected cards:', selectedCards.length);
    
    let allData = [];
    
    if (selectedCards.length > 0) {
      console.log('Booking Using data from selected cards');
      ...
      selectedCards.forEach(card => {
        ...
        if (cardData && cardData.length > 0) {
          allData = allData.concat(cardData);
          console.log(`Booking Added ${cardData.length} records from card "${card.name}"`);
        }
      });
    } else {
      console.log('Booking No cards selected, using original data');
      // Use original data as fallback when no cards are selected
      allData = this.getOriginalData();
    }

    console.log('Booking Total data records:', allData.length);

    if (allData.length === 0) {
      console.log('Booking ERROR: No data available!');
    ...
    console.log('Booking Available headers:', headers);
    
    duplicateColumnsList.innerHTML = '';
    console.log('Booking Cleared duplicateColumnsList');
    ...
    console.log('Booking Column stats calculated:', columnStats.length, 'columns');
    ...
    console.log('Booking Starting to create column items...');
    ...
    columnStats.forEach((stat, index) => {
      ...
      if (index < 5) { // Log first 5 items for debugging
        console.log(`Booking Created item ${index + 1}: "${stat.header}"`);
      }
    });

    console.log(`Booking Created ${columnStats.length} column items total`);
    console.log('Booking Final duplicateColumnsList children count:', duplicateColumnsList.children.length);
  ...
  setupDuplicateColumnsSearch() {
    ...
    if (searchInput) {
      ...
      this.handleDuplicateColumnsSearch = (e) => {
        ...
        console.log('🔍 Booking Search term:', searchTerm);
        ...
        console.log(`✅ Booking Filtered ${columnItems.length} checkboxes, ${visibleCount} visible`);
      };
  ...
  updateDuplicateColumns() {
    ...
    const selectedFieldsCount = document.getElementById('bookingSelectedFieldsCount');
  ...
  showDuplicateSummary() {
    const duplicateSummary = document.getElementById('bookingDuplicateSummary');
  ...
  _showDuplicateSummaryInternal(duplicateSummary) {
    try {
      ...
      console.error('Booking Error in duplicate analysis:', error);
