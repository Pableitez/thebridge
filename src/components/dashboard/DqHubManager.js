...
class DqHubManager {
        ...
        Object.entries(grouped).forEach(([key, group]) => {
            ...
            group.filters.forEach(({ name, filterObj }) => {
                ...
                const updateCardCount = () => {
                    ...
                    const activeFiltersArray = window.activeDqDashboardQuickFilters || [];
                    ...
                    if (activeFiltersSet.has(name)) {
                        ...
                        // USAR LA MISMA LÓGICA QUE getDqQuickFilterPreviewCount
                        // Aplicar temporalmente el filtro y usar getFilteredData() que ya tiene toda la lógica correcta
                        const allQuickFilters = this.loadDqQuickFilters();
                ...
                // Evento de click siempre activo, pero verifica compatibilidad dentro
                    ...
                    if (!sameColumns) {
                        ...
                        if (wasActive) {
                            ...
                            // Sincronizar array interno
                            if (!this.dqActiveFilters) this.dqActiveFilters = [];
                    ...
                    const updateAllCounts = () => {
                        ...
                        updateTimeout = setTimeout(() => {
                            const hubModal = document.querySelector('#dqDashboardModal');
                            ...
                            allCards.forEach((cardEl) => {
                                ...
                                if (cardEl._updateCount && typeof cardEl._updateCount === 'function') {
                                    try {
                                        ...
                                        console.error(`❌ [DQ] Error updating count for card ${cardName}:`, error);
                                    }
                                }
                            });
                            updateTimeout = null;
    ...
    toggleDqDashboard() {
        if (this.dqDashboardModal) {
            const isOpening = this.dqDashboardModal.classList.contains('hidden');
            this.dqDashboardModal.classList.toggle('hidden');
            if (!this.dqDashboardModal.classList.contains('hidden') && isOpening) {
