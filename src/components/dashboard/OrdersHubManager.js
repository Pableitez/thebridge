...
class OrdersHubManager {
            ...
            group.filters.forEach(({ name, filterObj }) => {
                ...
                const updateCardCount = () => {
                    ...
                    if (activeFiltersSet.has(name)) {
                        ...
                        // USAR LA MISMA LÓGICA QUE getOrdersQuickFilterPreviewCount
                        // Aplicar temporalmente el filtro y usar getFilteredData() que ya tiene toda la lógica correcta
                        const allQuickFilters = this.loadOrdersQuickFilters();
    ...
    updateOrdersMetrics() {
        ...
        this.updateMetric('orders-nulls', 'Null Rate', `${((nulls.total / (data.length * Object.keys(data[0] || {}).length)) * 100).toFixed(1)}%`);
        ...
        this.updateMetric('orders-format', 'Error Rate', `${((formatIssues.total / (data.length * Object.keys(data[0] || {}).length)) * 100).toFixed(1)}%`);
