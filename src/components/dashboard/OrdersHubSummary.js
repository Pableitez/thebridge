...
export class OrdersHubSummary {
    ...
    window.ordersFilterDuplicateColumns = () => {
      if (window.ordersHubSummaryInstance) {
        window.ordersHubSummaryInstance.filterDuplicateColumns();
      }
    };
  }
  ...
  bindEvents() {
    ...
    const generateBtn = document.getElementById('generateOrdersSummaryBtn');
    if (generateBtn) {
      generateBtn.addEventListener('click', () => this.openSummaryModal());
    }

    // Modal close button
    const closeBtn = document.getElementById('closeOrdersSummaryBtn');
    if (closeBtn) {
      closeBtn.addEventListener('click', () => this.closeSummaryModal());
    }

    // Action buttons
    const copyBtn = document.getElementById('ordersCopySummaryBtn');
    const pdfBtn = document.getElementById('ordersExportSummaryPdfBtn');
    const excelBtn = document.getElementById('ordersExportSummaryExcelBtn');
  ...
  setupModalHandlers() {
    const modal = document.getElementById('ordersSummaryModal');
    if (modal) {
  ...
  openSummaryModal() {
    console.log('Orders Opening summary modal...');
    
    // Expose instance globally for HTML event handlers
    window.ordersHubSummaryInstance = this;
    
    // Restore current state before opening
    this.restoreCurrentState();
    
    // Force refresh of quick filters data before collecting
    if (typeof window.renderOrdersDashboardQuickFilters === 'function') {
      console.log('Orders Refreshing quick filters...');
      window.renderOrdersDashboardQuickFilters();
    }
    
    // Wait a bit for the DOM to update, then collect data
    setTimeout(() => {
      console.log('Orders Setting up modal content...');
      ...
      console.log('Orders Setting up event listeners...');
      this.setupModalEventListeners();
      
      // FORCE POPULATE COLUMNS IMMEDIATELY
      console.log('Orders FORCING populateDuplicateColumns...');
      ...
      if (modal) {
        ...
        console.log('Orders Modal element not found');
      }
    }, 100);
  }

  setupModalEventListeners() {
    console.log('Setting up Orders modal event listeners...');
    ...
    // Duplicate removal
      ...
      console.log('Orders Found removeDuplicatesCheckbox, setting up event listener');
            removeDuplicatesCheckbox.addEventListener('change', () => {
        console.log('Orders Checkbox change event triggered');
        this.handleDuplicateRemovalChange();
      });
    } else {
      console.log('Orders ERROR: removeDuplicatesCheckbox not found');
    }

    // Duplicate columns controls
    const selectAllColumnsBtn = document.getElementById('ordersSelectAllDuplicateColumnsBtn');
    const deselectAllColumnsBtn = document.getElementById('ordersDeselectAllDuplicateColumnsBtn');
    const selectCommonFieldsBtn = document.getElementById('ordersSelectCommonFieldsBtn');
  ...
  saveCurrentState() {
    try {
      ...
      localStorage.setItem('ordersSummaryCurrentState', JSON.stringify(currentState));
      console.log('Orders Current state saved:', currentState);
    } catch (error) {
      console.error('Orders Error saving current state:', error);
    }
  }
  
  restoreCurrentState() {
    try {
      const savedState = localStorage.getItem('ordersSummaryCurrentState');
      if (savedState) {
        ...
        console.log('Orders Current state restored:', state);
        return true;
      }
    } catch (error) {
      console.error('Orders Error restoring current state:', error);
    }
    return false;
  }
  ...
  collectQuickCardsData() {
    console.log('Collecting Orders quick cards data...');
    this.quickCardsData = {};
    this.currentPriority = '';

    // Get current priority filter
    const activeChip = document.querySelector('#ordersDashboardModal .orders-hub-chip.active');
    if (activeChip) {
      this.currentPriority = activeChip.getAttribute('data-urgency');
      console.log('Current Orders priority filter:', this.currentPriority);
    } else {
      console.log('No active Orders priority filter found');
    }

    // Collect data from quick cards - look for kpi-card elements in quickfilter containers
    let quickCards = document.querySelectorAll('#ordersDashboardModal .quickfilter-cards-container .kpi-card');
    
    // If no cards found, try alternative selectors
    if (quickCards.length === 0) {
      quickCards = document.querySelectorAll('#ordersDashboardModal .quickfilters-grid .kpi-card');
    }
    
    // If still no cards, try any kpi-card in the Orders dashboard
    if (quickCards.length === 0) {
      quickCards = document.querySelectorAll('#ordersDashboardModal .kpi-card');
    }
    ...
    quickCards.forEach(card => {
      ...
      if (!cardName) {
        cardName = 'Orders Quick Filter Card';
      }
    ...
    if (Object.keys(this.quickCardsData).length === 0) {
      this.quickCardsData['no_cards'] = {
        id: 'no_cards',
        name: 'No Orders Quick Cards Available',
        count: 0,
        active: false,
        element: null
      };
    ...
    console.log('Collected Orders quick cards data:', this.quickCardsData);
    console.log('Total Orders cards found:', Object.keys(this.quickCardsData).length);
  }

  generateCardId(card) {
    const title = card.querySelector('.kpi-title')?.textContent || '';
    return `orders_card_${title.toLowerCase().replace(/[^a-z0-9]/g, '_')}_${Date.now()}`;
  }

  getOrdersQuickFilterPreviewCount(filterName) {
    // Get preview count for Orders quick filter
    if (typeof window.getOrdersQuickFilterPreviewCount === 'function') {
      return window.getOrdersQuickFilterPreviewCount(filterName);
    }
    return '0';
  }
  ...
  updateSummaryInfo() {
    ...
    console.log(`Orders Summary Info: ${activeCards}/${totalCards} cards active`);
  }

  renderCardsSelection() {
    const cardsContainer = document.getElementById('ordersQuickCardsSelection');
  ...
  _renderSummaryPreviewInternal() {
    const preview = document.getElementById('ordersSummaryPreview');
  ...
  generateSeparateTablesForCards(selectedCards, config) {
      ...
      if (cardData && cardData.length > 0) {
        ...
        if (cardConfig.viewType === 'summary') {
          ...
          if (config.includeTableData) {
            console.log(`Orders Generating table for card "${card.name}" with ${processedData.length} records`);
            const tableConfig = {
              ...cardViewConfig,
        ...
        console.log(`Orders No data available for card "${card.name}"`);
        tablesHtml += `
          <div style="margin-bottom:2rem;">
            <h3 style="font-size:1.1rem; color:#15364A; margin-bottom:0.5rem; border-bottom:1px solid #e3f2fd; padding-bottom:0.3rem;">
              ${card.name}
    ...
    console.log(`Orders generateSeparateTablesForCards completed: ${totalRecords} total records from ${selectedCards.length} cards`);
  ...
  generateTableDataSection(tableData, config, customTitle = 'Table Data') {
    console.log(`Orders generateTableDataSection called for "${customTitle}":`, {
    ...
    if (config.savedView && config.savedView !== 'current') {
      ...
      if (selectedView && selectedView.columns) {
        console.log(`Orders Applying saved view columns "${config.savedView}" to table:`, selectedView.columns);
        ...
        console.log(`Orders Filtered headers for saved view:`, headers);
      }
    }
  ...
  getDataForCurrentConfiguration(config) {
    ...
    // If using a saved view, apply that view to the data
      ...
      if (selectedView && selectedView.columns) {
        console.log(`Orders Applying saved view "${config.tableView}" to current configuration:`, selectedView);
        ...
        console.log(`Orders Data after applying saved view "${config.tableView}":`, data.length, 'records');
        result = data;
      } else {
        console.warn(`Orders Saved view "${config.tableView}" not found or invalid.`);
        result = this.getOriginalData(); // Use original data, not filtered
      }
    }
    // Default: use original data for Orders Hub (PRUEBA)
    else {
      result = this.getOriginalData(); // Usar los datos originales para Orders Hub
    }
    
    // Apply duplicate removal if enabled
    if (config.removeDuplicates && config.duplicateColumns.length > 0) {
      console.log('Orders Applying duplicate removal to current configuration data');
  ...
  getDataForQuickCardWithConfig(cardData, cardConfig) {
    // PRUEBA: Usar datos originales en lugar de filtrados para Orders Hub
    console.log('Orders getDataForQuickCardWithConfig called for:', cardData.name);
    console.log('Orders window.getFilteredData available:', !!window.getFilteredData);
    console.log('Orders window.getOriginalData available:', !!window.getOriginalData);
    
    // PRUEBA: Usar datos originales para Orders Hub
    let filteredData = this.getOriginalData();
    console.log(`Orders Initial filteredData length: ${filteredData.length}`);
    
    // Apply saved view if specified and not "current"
    if (cardConfig && cardConfig.savedView && cardConfig.savedView !== 'current') {
      console.log(`Orders Applying saved view "${cardConfig.savedView}" to card "${cardData.name}"`);
      
      const savedViews = this.loadSavedViews();
      const selectedView = savedViews[cardConfig.savedView];
      
      if (selectedView && selectedView.columns) {
        console.log(`Orders Found saved view "${cardConfig.savedView}":`, selectedView);
        ...
        console.log(`Orders Applied saved view "${cardConfig.savedView}": ${filteredData.length} records have values in all specified columns`);
            } else {
        console.warn(`Orders Saved view "${cardConfig.savedView}" not found or invalid`);
      }
    }
    
    console.log(`Orders Looking for quick filter for card: "${cardData.name}" (ID: ${cardData.id})`);
    
    // Use Orders-specific quick filters instead of general quick filters
    console.log('Orders window.ordersHubManager available:', !!window.ordersHubManager);
    const ordersQuickFilters = window.ordersHubManager ? window.ordersHubManager.loadOrdersQuickFilters() : {};
    console.log('Orders Available Orders quick filters:', Object.keys(ordersQuickFilters));
    console.log('Orders Orders quick filters content:', ordersQuickFilters);
    
    // Find the quick filter that corresponds to this card
    const filterEntry = Object.entries(ordersQuickFilters).find(([name, obj]) => {
      console.log(`Orders Checking filter "${name}":`, obj);
      ...
      if (obj.linkedUrgencyCard && obj.linkedUrgencyCard === cardData.id) {
        console.log(`Orders ✓ Matched by linkedUrgencyCard: ${obj.linkedUrgencyCard} === ${cardData.id}`);
        return true;
      }
      
      // Priority 2: Match by exact name
      if (obj.name && obj.name === cardData.name) {
        console.log(`Orders ✓ Matched by exact name: "${obj.name}" === "${cardData.name}"`);
        return true;
      }
      
      // Priority 3: Match by filter name containing card name
      if (name.toLowerCase().includes(cardData.name.toLowerCase())) {
        console.log(`Orders ✓ Matched by filter name contains card name: "${name}" contains "${cardData.name}"`);
        return true;
      }
      
      // Priority 4: Match by obj.name containing card name
      if (obj.name && obj.name.toLowerCase().includes(cardData.name.toLowerCase())) {
        console.log(`Orders ✓ Matched by obj.name contains card name: "${obj.name}" contains "${cardData.name}"`);
        return true;
      }
      ...
      if (cardUrgency) {
        ...
        if (matchesUrgency) {
          console.log(`Orders ✓ Matched by urgency level: "${cardData.name}" matches "${name}" or "${obj.name}"`);
          return true;
        }
      }
    ...
    if (filterEntry) {
      ...
      console.log(`Orders ✓ Found matching quick filter: "${filterName}" for card "${cardData.name}"`);
      console.log('Orders Filter object:', filterObj);
      ...
      console.log(`Orders Card shows ${cardData.count} records, filter returned ${bestResult.length} records`);
      if (cardData.count !== bestResult.length) {
        console.warn(`Orders ⚠️ Count mismatch: Card shows ${cardData.count} but filter returned ${bestResult.length}`);
      }
      
      console.log(`Orders Returning bestResult with ${bestResult.length} records for card "${cardData.name}"`);
      return bestResult;
    }

    // Fallback to filtered data (not original) - EXACTAMENTE como Ops Hub
    console.log(`Orders ❌ No quick filter found for card "${cardData.name}", using filtered data`);
    console.log(`Orders Returning filteredData with ${filteredData.length} records for card "${cardData.name}"`);
    return filteredData;
  }

  applyQuickFilterWithSystemLogic(filterName, filterObj, baseData) {
    // Soporta quick filters de duplicados para el preview del Orders Summary
    const data = baseData;
    ...
    if (!window.getModuleActiveFilters || !window.setModuleActiveFilters || 
      ...
      console.warn('Orders System filter functions not available, using fallback method');
    ...
    console.log(`Orders Quick filter "${filterName}" applied with system logic: ${data.length} -> ${count} records`);
  ...
  applyQuickFilterToData(filterName, filterObj, baseData) {
    ...
    if (!filterObj || !filterObj.filterValues) {
      console.log(`Orders No filterValues in filterObj for "${filterName}"`, filterObj);
      return originalData;
    }
    ...
    Object.entries(filterObj.filterValues).forEach(([column, filterValue]) => {
      if (filterValue && filterValue !== '') {
        ...
        console.log(`Orders Filter "${column} = ${filterValue}": ${beforeCount} -> ${afterCount} records`);
      }
    });
    
    console.log(`Orders Quick filter "${filterName}" applied (fallback): ${originalData.length} -> ${filteredData.length} records`);
    return filteredData;
  }

  getOriginalData() {
    console.log('Orders Getting original data...');
    ...
    if (window.getOriginalData && typeof window.getOriginalData === 'function') {
      ...
      console.log(`Orders Got original data from window.getOriginalData(): ${data.length} records`);
      return data;
    }
    ...
    if (table) {
      ...
      if (rows.length > 0) {
        ...
        console.log(`Orders Got original data from table: ${data.length} records`);
        return data;
      }
    }
    ...
    if (window.tableData && Array.isArray(window.tableData)) {
      console.log(`Orders Got original data from window.tableData: ${window.tableData.length} records`);
      return window.tableData;
    }
    
    // Try to get from CSV data if available
    if (window.csvData && Array.isArray(window.csvData)) {
      console.log(`Orders Got original data from window.csvData: ${window.csvData.length} records`);
      return window.csvData;
    }
    
    console.warn('Orders Could not get original data, using empty array');
    return [];
  }
  ...
  updateCardConfig(cardId, key, value) {
    ...
    console.log(`Orders Updated card config for ${cardId}:`, this.cardConfigs[cardId]);
  }
  ...
  loadSavedViews() {
    try {
      ...
      console.error('Orders Error loading saved views:', error);
      return {};
    }
  }
    ...
    try {
      ...
      if (navigator.clipboard && navigator.clipboard.write) {
        try {
          ...
          this.showNotification('Orders summary copied to clipboard with formatting!', 'success');
          return;
        } catch (htmlError) {
  ...
  exportToExcel() {
    try {
      ...
      const configData = [
        { 'Setting': 'Report Type', 'Value': 'Orders Summary Report' },
      ...
      // Save the file
      const fileName = `Orders_Summary_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  generateCleanTextContent() {
    const selectedCards = Array.from(this.selectedCards);
    if (selectedCards.length === 0) {
      return 'No cards selected for Orders summary report.';
    }

    const config = this.getReportConfig();
    const dateStr = new Date().toLocaleString();
    
    let content = `Orders Summary Report\n`;
  ...
  removeDuplicatesFromData(data, config = null) {
    ...
    console.log('Orders removeDuplicatesFromData called with:', {
    ...
    if (!reportConfig.removeDuplicates || reportConfig.duplicateColumns.length === 0) {
      console.log('Orders Duplicate removal not enabled or no columns selected, returning original data');
      return data;
    }
    ...
    console.log(`Orders Duplicate removal applied: ${data.length} -> ${filteredData.length} records (removed ${data.length - filteredData.length})`);
    return filteredData;
  }
  ...
  populateDuplicateColumns() {
    const duplicateColumnsList = document.getElementById('ordersDuplicateColumnsList');
    
    if (!duplicateColumnsList) {
      console.log('Orders ERROR: duplicateColumnsList element not found!');
      return;
    }

    console.log('Orders populateDuplicateColumns called - starting...');

    // Get data from selected cards or use original data as fallback
    const selectedCards = Object.values(this.quickCardsData).filter(card => this.selectedCards.has(card.id));
    console.log('Orders Selected cards:', selectedCards.length);
    
    let allData = [];
    
    if (selectedCards.length > 0) {
      console.log('Orders Using data from selected cards');
      ...
      selectedCards.forEach(card => {
        ...
        if (cardData && cardData.length > 0) {
          allData = allData.concat(cardData);
          console.log(`Orders Added ${cardData.length} records from card "${card.name}"`);
        }
      });
    } else {
      console.log('Orders No cards selected, using original data');
      // Use original data as fallback when no cards are selected
      allData = this.getOriginalData();
    }

    console.log('Orders Total data records:', allData.length);

    if (allData.length === 0) {
      console.log('Orders ERROR: No data available!');
    ...
    console.log('Orders Available headers:', headers);
    
    duplicateColumnsList.innerHTML = '';
    console.log('Orders Cleared duplicateColumnsList');
    ...
    console.log('Orders Column stats calculated:', columnStats.length, 'columns');
    ...
    console.log('Orders Starting to create column items...');
    ...
    columnStats.forEach((stat, index) => {
      ...
      if (index < 5) { // Log first 5 items for debugging
        console.log(`Orders Created item ${index + 1}: "${stat.header}"`);
      }
    });

    console.log(`Orders Created ${columnStats.length} column items total`);
    console.log('Orders Final duplicateColumnsList children count:', duplicateColumnsList.children.length);
  ...
  setupDuplicateColumnsSearch() {
    ...
    if (searchInput) {
      ...
      this.handleDuplicateColumnsSearch = (e) => {
        ...
        console.log('🔍 Orders Search term:', searchTerm);
        ...
        console.log(`✅ Orders Filtered ${columnItems.length} checkboxes, ${visibleCount} visible`);
      };
  ...
  updateDuplicateColumns() {
    ...
    const selectedFieldsCount = document.getElementById('ordersSelectedFieldsCount');
  ...
  showDuplicateSummary() {
    const duplicateSummary = document.getElementById('ordersDuplicateSummary');
  ...
  _showDuplicateSummaryInternal(duplicateSummary) {
    try {
      ...
      console.error('Orders Error in duplicate analysis:', error);
