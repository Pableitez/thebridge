  ...
  try {
            ...
            <div style="background:#fff;padding:2.2em 2.5em 1.5em 2.5em;border-radius:12px;box-shadow:0 4px 32px #0002;min-width:320px;max-width:90vw;">
              ...
              <div style='margin-bottom:1em;'>
                ...
                <select id='quickFilterContainerSelect' style='width:100%;padding:0.5em;margin-top:0.2em;border-radius:6px;border:1px solid #ccc;font-size:1em;'>
                  <option value='default'>Default Filters</option>
                  <option value='container1'>KPIs</option>
                  <option value='container2'>Saved Queries</option>
                  <option value='container3'>Custom Reports</option>
                  <option value='container4'>Favorites</option>
                </select>
    ...
    function renderMyFiltersSection() {
      ...
      Object.entries(filters).forEach(([name, obj]) => {
        ...
        if (obj.headerHash !== headerHash) {
          applyBtn.disabled = true;
          applyBtn.title = 'This filter does not match the current columns.';
          item.title = '';
          item.classList.add('my-filter-item-incompatible');
          // Permitir pointer-events para mostrar el tooltip
          item.style.pointerEvents = 'auto';
          // Tooltip personalizado al pasar el mouse por cualquier parte
          item.addEventListener('mouseenter', (e) => {
            let tooltip = document.getElementById('myFilterTooltip');
            if (!tooltip) {
              tooltip = document.createElement('div');
              tooltip.id = 'myFilterTooltip';
              tooltip.style.position = 'fixed';
              tooltip.style.zIndex = 9999;
              tooltip.style.background = '#f5f6fa';
            ...
            tooltip.textContent = 'Incompatible with current CSV. You can only delete it.';
          ...
          // El botón Delete debe estar activo y visible
...
function applyQuickFilter(name) {
  // Desactiva cualquier tarjeta de urgencia activa
  if (window.activeUrgencyCards) window.activeUrgencyCards = [];
  // Quita la clase activa de las tarjetas de urgencia en la UI
  document.querySelectorAll('.urgency-card.active, .urgency-card[aria-pressed="true"]').forEach(card => {
  ...
  if (filterObj) {
    ...
    if (filterModal && filterOverlay) {
      ...
      filterOverlay.classList.remove('blur'); // Por si acaso
      filterOverlay.style.backdropFilter = 'none'; // Forzar quitar blur
    }
    ...
    // Quitar overlay de dashboard si existe
    const dashboardOverlay = document.querySelector('.dashboard-overlay');
    if (dashboardOverlay) {
      dashboardOverlay.style.display = 'none';
      dashboardOverlay.classList.remove('visible');
      dashboardOverlay.classList.remove('blur');
      dashboardOverlay.style.backdropFilter = 'none';
    }
  }
}
