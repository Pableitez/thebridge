...
function exportPivotToExcel() {
  ...
  const ws_data = [];
  if (legend) ws_data.push([legend]);
  ws_data.push(headers);
  rowsData.forEach(row => ws_data.push(row));
  const ws = XLSX.utils.aoa_to_sheet(ws_data);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Summary');
  XLSX.writeFile(wb, 'custom-summary.xlsx');
...
export function setupSummaryModalEvents() {
  ...
  // Botón para ver resumen visual
  if (viewBtn) viewBtn.onclick = openCopySummaryModal;
}
...
function openCopySummaryModal() {
  ...
  // Leyenda de filtros
  const legend = getActiveFiltersLegend();
  // Generar resumen visual como tabla HTML
  const rows = getPivotRows();
  const col = getPivotColumn();
  const metrics = getSelectedMetrics();
  const orderByField = getOrderByField();
  ...
  // Cerrar modal de resumen visual
  setTimeout(() => {
