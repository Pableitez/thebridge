...
export function groupAndSummarize(data, groupByFields, metrics) {
  ...
  return Object.entries(groups).map(([groupValue, rows]) => {
    ...
    groupByFields.forEach((f, idx) => {
      pivot[f] = groupValue.split(' | ')[idx];
    });
    metrics.forEach(metric => {
      if (metric.type === 'count') {
        pivot[`Count of ${metric.field}`] = rows.length;
      }
      if (metric.type === 'sum') {
        pivot[`Sum of ${metric.field}`] = rows.reduce((acc, r) => acc + (parseFloat(r[metric.field]) || 0), 0);
      }
      if (metric.type === 'avg') {
        pivot[`Avg of ${metric.field}`] = rows.length
          ? rows.reduce((acc, r) => acc + (parseFloat(r[metric.field]) || 0), 0) / rows.length
          : 0;
      }
      if (metric.type === 'min') {
        pivot[`Min of ${metric.field}`] = Math.min(...rows.map(r => parseFloat(r[metric.field]) || Infinity));
      }
      if (metric.type === 'max') {
        pivot[`Max of ${metric.field}`] = Math.max(...rows.map(r => parseFloat(r[metric.field]) || -Infinity));
      }
      if (metric.type === 'unique') {
        pivot[`Unique count of ${metric.field}`] = new Set(rows.map(r => r[metric.field])).size;
      }
      if (metric.type === 'concat') {
        ...
        pivot[`List of ${metric.field}`] = uniqueVals.length > 0 ? `(${uniqueVals.length}): ${uniqueVals.join(', ')}` : '(sin datos)';
      }
    });
    return pivot;
  });
...
export function openPivotModal() {
  try {
    console.log('🚀 openPivotModal iniciado');
    const headers = getVisibleColumns();
    console.log('Opening pivot modal with headers:', headers);
    ...
    setTimeout(() => {
      const closeBtn = document.getElementById('closePivotModalBtn');
      if (closeBtn) {
    ...
    document.getElementById('pivotModal').classList.remove('hidden');
    console.log('📋 Modal abierto, configurando eventos...');
    setupManageSummariesEvents();
    setupPivotModalEvents();
    console.log('✅ Eventos configurados');
    document.getElementById('resetPivotBtn').addEventListener('click', resetPivot);
    
    // Forzar conexión del botón Excel después de un delay
    setTimeout(() => {
      console.log('🔧 Forzando conexión del botón Excel después de abrir modal...');
      const excelBtn = document.getElementById('exportPivotExcelBtn');
      if (excelBtn) {
    ...
    setTimeout(() => {
      const resetBtn = document.getElementById('resetPivotBtn');
      if (resetBtn) resetBtn.onclick = resetPivot;
    }, 0);
  } catch (error) {
    console.error('Error opening pivot modal:', error);
  }
}
...
function getPivotTableDataAndHeaders(data, rows, col, metrics, orderByField, removeDupFields = []) {
...
function generatePivotTablePreview(data, rows, col, metrics, orderByField) {
  ...
  const { headers, rowsData, totalsRow } = getPivotTableDataAndHeaders(data, rows, col, metrics, orderByField, removeDupFields);
...
export function showPivotPDF(resumen, groupByFields, metrics, orderByField = groupByFields[0]) {
  ...
  addLogoAndTitle(() => {
    ...
    doc.text('Custom Pivot', marginLeft, 30);
    doc.setFontSize(12);
    ...
    doc.save('custom-pivot.pdf');
  });
}

function exportCustomPivotToExcel() {
  try {
    console.log('[CustomPivot] Export to Excel iniciado');
    if (!window.XLSX) {
      console.error('[CustomPivot] ❌ XLSX no está disponible');
  ...
  const orderByField = getOrderByField();
    ...
    if (!headers.length || !finalRowsData.length) {
      console.warn('[CustomPivot] No hay datos para exportar');
      alert('No hay datos para exportar a Excel.');
      return;
    }
    const ws_data = [headers, ...finalRowsData];
    console.log('[CustomPivot] Datos para Excel:', ws_data);
  const ws = XLSX.utils.aoa_to_sheet(ws_data);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Pivot');
    const filename = `custom_pivot_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(wb, filename);
    console.log('[CustomPivot] Excel exportado exitosamente:', filename);
  } catch (error) {
    console.error('[CustomPivot] Error al exportar a Excel:', error);
...
export function setupPivotModalEvents() {
  console.log('setupPivotModalEvents ejecutado');
  
  // Verificar y conectar botón de Excel
  const excelBtn = document.getElementById('exportPivotExcelBtn');
  if (!excelBtn) {
    console.error('❌ No se encontró el botón Export to Excel (id=exportPivotExcelBtn)');
    ...
    setTimeout(() => {
      ...
      const retryBtn = document.getElementById('exportPivotExcelBtn');
      if (retryBtn) {
    ...
    excelBtn.removeEventListener('click', exportCustomPivotToExcel);
  ...
  const pdfBtn = document.getElementById('generatePivotBtn');
  if (!pdfBtn) {
    console.error('No se encontró el botón Generate PDF (id=generatePivotBtn)');
  } else {
  ...
  const viewBtn = document.getElementById('viewPivotBtn');
  if (!viewBtn) {
    console.error('No se encontró el botón View pivot (id=viewPivotBtn)');
  } else {
    console.log('Botón View pivot encontrado, asignando evento');
    viewBtn.onclick = openCopyPivotModal;
  }

  // Cierre al hacer click en el overlay (blur)
  const pivotModal = document.getElementById('pivotModal');
  if (pivotModal) {
    pivotModal.addEventListener('mousedown', function(e) {
      if (e.target === pivotModal) {
  ...
  if (closePivotModalBtn) {
    console.log('✅ Configurando botón de cerrar del pivot modal');
    
    // Limpiar eventos previos
    closePivotModalBtn.onclick = null;
    closePivotModalBtn.removeEventListener('click', closePivotModal);
    
    // Asignar nuevo evento
    closePivotModalBtn.onclick = function(e) {
      console.log('🎯 CLICK en botón cerrar del pivot modal');
      e.preventDefault();
      e.stopPropagation();
      if (pivotModal) {
        pivotModal.classList.add('hidden');
    ...
    closePivotModalBtn.addEventListener('click', function(e) {
      console.log('🎯 CLICK (addEventListener) en botón cerrar del pivot modal');
      e.preventDefault();
      e.stopPropagation();
      if (pivotModal) {
        pivotModal.classList.add('hidden');
    ...
    console.error('❌ No se encontró el botón de cerrar del pivot modal (closePivotModalBtn)');
  }
  
  // Función para cerrar el modal
  function closePivotModal() {
    console.log('🔒 Cerrando pivot modal');
    if (pivotModal) {
      pivotModal.classList.add('hidden');
    }
  }
  ...
  window.closePivotModal = closePivotModal;
  
  // Configurar botón de cerrar del copy pivot modal como respaldo
  const closeCopyPivotModalBtn = document.getElementById('closeCopyPivotModalBtn');
  if (closeCopyPivotModalBtn) {
    console.log('✅ Configurando botón de cerrar del copy pivot modal (respaldo)');
    
    // Limpiar eventos previos
    closeCopyPivotModalBtn.onclick = null;
    closeCopyPivotModalBtn.removeEventListener('click', closeCopyPivotModal);
    
    // Asignar nuevo evento
    closeCopyPivotModalBtn.onclick = function(e) {
      console.log('🎯 CLICK en botón cerrar del copy pivot modal (respaldo)');
      e.preventDefault();
      e.stopPropagation();
      const modal = document.getElementById('copyPivotModal');
      if (modal) {
        modal.classList.add('hidden');
    ...
    // También agregar event listener como respaldo
    closeCopyPivotModalBtn.addEventListener('click', function(e) {
      console.log('🎯 CLICK (addEventListener) en botón cerrar del copy pivot modal (respaldo)');
      e.preventDefault();
      e.stopPropagation();
      const modal = document.getElementById('copyPivotModal');
      if (modal) {
    ...
    console.log('✅ Botón de cerrar del copy pivot modal configurado correctamente (respaldo)');
  } else {
    console.error('❌ No se encontró el botón de cerrar del copy pivot modal (closeCopyPivotModalBtn) en respaldo');
  }
  
  // Función para cerrar el copy pivot modal
  function closeCopyPivotModal() {
    console.log('🔒 Cerrando copy pivot modal (respaldo)');
    const modal = document.getElementById('copyPivotModal');
    if (modal) {
      modal.classList.add('hidden');
      modal.style.display = 'none';
  ...
  // Hacer la función disponible globalmente
  window.closeCopyPivotModal = closeCopyPivotModal;
...
function openCopyPivotModal() {
  console.log('openCopyPivotModal ejecutado');
  // Leyenda de filtros
  const legend = getActiveFiltersLegend();
  ...
  document.getElementById('copyPivotContent').innerHTML = html;
  ...
  if (!finalRowsData || !finalRowsData.length) {
    document.getElementById('copyPivotContent').innerHTML = '<div style="color:#d32f2f;padding:1em;">No data to copy.</div>';
    modal.classList.remove('hidden');
    modal.style.display = 'flex';
...
function setupCopyModalButtons() {
  ...
  // Configurar botón de cerrar
    const closeBtn = document.getElementById('closeCopyPivotModalBtn');
  if (closeBtn) {
    console.log('✅ Configurando botón de cerrar del copy pivot modal');
    
    // Limpiar eventos previos
    closeBtn.onclick = null;
    closeBtn.removeEventListener('click', closeCopyPivotModal);
    
    // Asignar nuevo evento
    closeBtn.onclick = function(e) {
      console.log('🎯 CLICK en botón cerrar del copy pivot modal');
      e.preventDefault();
      e.stopPropagation();
      const modal = document.getElementById('copyPivotModal');
      if (modal) {
    ...
    closeBtn.addEventListener('click', function(e) {
      console.log('🎯 CLICK (addEventListener) en botón cerrar del copy pivot modal');
      e.preventDefault();
      e.stopPropagation();
        const modal = document.getElementById('copyPivotModal');
      if (modal) {
    ...
    console.log('✅ Botón de cerrar del copy pivot modal configurado correctamente');
  } else {
    console.error('❌ No se encontró el botón de cerrar del copy pivot modal (closeCopyPivotModalBtn)');
  }
  
  // Configurar botón de copia
  const copyBtn = document.getElementById('copyPivotToClipboardBtn');
  if (copyBtn) {
    console.log('✅ Configurando botón de copia del copy pivot modal');
          
    // Limpiar eventos previos
    copyBtn.onclick = null;
    copyBtn.removeEventListener('click', copyPivotToClipboard);
          
    // Asignar nuevo evento
    copyBtn.onclick = copyPivotToClipboard;
    
    console.log('✅ Botón de copia del copy pivot modal configurado correctamente');
        } else {
    console.error('❌ No se encontró el botón de copia del copy pivot modal (copyPivotToClipboardBtn)');
  }
  
  // Función para cerrar el copy pivot modal
  function closeCopyPivotModal() {
    console.log('🔒 Cerrando copy pivot modal');
    const modal = document.getElementById('copyPivotModal');
    if (modal) {
      modal.classList.add('hidden');
      modal.style.display = 'none';
  ...
  async function copyPivotToClipboard() {
    ...
    try {
      ...
      const modal = document.getElementById('copyPivotModal');
      const content = document.getElementById('copyPivotContent');
      ...
      try {
        ...
        if (typeof window.showUnifiedNotification === 'function') {
          window.showUnifiedNotification('Pivot copied to clipboard successfully!', 'success');
        }
        
        // Cambiar texto del botón temporalmente
        const btn = document.getElementById('copyPivotToClipboardBtn');
        if (btn) {
      ...
      // Mostrar notificación de error
      if (typeof window.showUnifiedNotification === 'function') {
        window.showUnifiedNotification('Failed to copy pivot to clipboard. Please try again.', 'error');
      }
    }
  }
  ...
  window.closeCopyPivotModal = closeCopyPivotModal;
  window.copyPivotToClipboard = copyPivotToClipboard;
}

// --- Gestión de Summaries ---
const PIVOT_STORAGE_KEY = 'customSummaries';
function getSavedSummaries() {
  try {
    return JSON.parse(localStorage.getItem(PIVOT_STORAGE_KEY)) || {};
  } catch { return {}; }
}
function saveSummaries(summaries) {
  localStorage.setItem(PIVOT_STORAGE_KEY, JSON.stringify(summaries));
...
function promptSavePivot() {
  const name = prompt('Enter a name for this pivot:');
  if (!name) return;
  const summaries = getSavedSummaries();
  summaries[name] = getCurrentPivotConfig();
...
function openManageSummariesModal() {
  ...
  Object.entries(summaries).forEach(([name, cfg]) => {
    ...
    const pivotName = document.createElement('span');
    pivotName.className = 'saved-view-name';
    pivotName.textContent = name;
    ...
    deleteBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      if (confirm(`Are you sure you want to delete the pivot "${name}"?`)) {
  ...
  document.getElementById('pivotRenameSection').style.display = 'none';
  modal.classList.remove('hidden');
  modal.style.display = 'flex';
}
function showRenameSection(oldName) {
  const input = document.getElementById('renamePivotInput');
  input.value = oldName;
  document.getElementById('pivotRenameSection').style.display = '';
  input.focus();
  document.getElementById('confirmRenamePivotBtn').onclick = function() {
...
function setupManageSummariesEvents() {
  ...
  const saveBtn = document.getElementById('savePivotBtn');
  if (saveBtn) saveBtn.onclick = promptSavePivot;
...
(function delegateExportExcelClick() {
  ...
  modal.addEventListener('click', function(e) {
    const btn = e.target.closest('#exportPivotExcelBtn');
    if (btn) {
      e.preventDefault();
      e.stopPropagation();
