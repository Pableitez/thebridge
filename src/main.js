...
async function handleFileUpload(event) {
  ...
  try {
    ...
    if (typeof setupViewSelect === 'function') setupViewSelect();
...
function renderDashboardQuickFilters() {
  ...
  const defaultContainers = {
    ...
    'container1': 'KPIs',
    'container2': 'Saved Queries',
    'container3': 'Custom Reports',
    'container4': 'Favorites'
  };
  ...
  Object.entries(quickFilters).forEach(([name, filterObj]) => {
    const key = filterObj.container || 'default';
    if (!grouped[key]) {
      grouped[key] = { 
  ...
  Object.entries(grouped).forEach(([key, group]) => {
    ...
    if (group.filters.length === 0) {
      ...
      group.filters.forEach(({ name, filterObj }) => {
        ...
        value.textContent = sameColumns ? getQuickFilterCount(filterObj.filterValues) : '-';
        ...
        if (sameColumns) {
          card.addEventListener('click', () => applyQuickFilter(name));
        }
...
function getQuickFilterCount(filterValues) {
  const data = getOriginalData();
...
function renderOpsHubFilterChips() {
  ...
  if (!chipsBar) {
    ...
    // Inserta los chips justo después del header
  ...
  urgencyCards.forEach(card => {
    ...
    btn.style.background = isActive ? '#0B36AD' : (filterEntry ? card.color : '#f5f6fa');
    btn.style.color = isActive ? '#fff' : (filterEntry ? '#222' : '#aaa');
    btn.style.border = isActive ? '2.5px solid #0B36AD' : '1.5px solid #b3c6e6';
    btn.style.borderRadius = '18px';
    btn.style.padding = '0.4em 1.2em';
    btn.style.fontWeight = 'bold';
    btn.style.cursor = (filterEntry && compatible) ? 'pointer' : 'not-allowed';
    btn.style.boxShadow = isActive ? '0 2px 12px #0B36AD33' : '0 1px 4px rgba(0,36,61,0.06)';
...
function applyOpsHubQuickFilters() {
  ...
  // Recolectar los filtros de cada tarjeta activa
  let combinedFilterValues = {};
  let combinedActiveFilters = {};
  activeCards.forEach(cardKey => {
      ...
      for (const key in filterObj.filterValues) {
        ...
        if (combinedFilterValues[key]) {
          if (Array.isArray(combinedFilterValues[key]) || Array.isArray(value)) {
            ...
            // Si ambos son escalares y distintos, convertir a array
