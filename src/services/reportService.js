...
async function generateExcelReport() {
  const { headers, rows } = getVisibleTableDataFromDOM();
  // 1. Prepara los datos para SheetJS
  const ws_data = [
  ...
  // 3. Estilo: encabezado azul, letras blancas, negrita
...
function setupReportService() {
  ...
  if (reportBtn) {
    reportBtn.addEventListener('click', () => {
      // Rellenar select y Choices.js justo antes de mostrar el modal
      if (dedupeColumnsSelect) {
        // Destruir Choices anterior si existe
        if (dedupeChoices) {
