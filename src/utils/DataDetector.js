...
class DataDetector {
  ...
  #detectColumnType(values, columnName) {
    ...
    // 1. Si el nombre sugiere fecha, prioriza fecha
    if (this.#isLikelyDateColumnByName(columnName, sample) && this.isDateColumn(values)) {
      return 'date';
    }

    // 1b. Forzar fecha si el nombre contiene 'UTC'
    ...
    // 2. Si el nombre sugiere referencia o los valores lo parecen, referencia
    if (this.#isReferenceColumn(columnName, sample)) {
      return 'reference';
    }

    // 3. Check other specific types
    ...
    // 4. Check if all values are numbers
    ...
    // 5. Check if all values are booleans
